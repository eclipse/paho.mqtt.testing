...
class MQTTClients:
	...
	def __eq__(self, a):
		...
		try:
			assert self.id == a.id
			assert self.cleansession == a.cleansession
			assert self.socket == a.socket
			assert self.msgid == a.msgid
			assert self.outbound == a.outbound
			assert self.outmsgs == a.outmsgs
			#assert self.broker == a.broker
			assert self.inbound == a.inbound
			assert self.connected == a.connected
			assert self.will == a.will
			assert self.keepalive == a.keepalive
			assert self.lastPacket == a.lastPacket
			return True
		except:
			traceback.print_exc()
			raise
...
class MQTTBrokers:
	...
	def __eq__(self, aBroker):
		if aBroker == None:
			return False
		try:
			assert self.publish_on_pubrel == aBroker.publish_on_pubrel
			assert self.dropQoS0 == aBroker.dropQoS0
			assert self.zero_length_clientids == aBroker.zero_length_clientids
			assert self.broker == aBroker.broker
			assert self.clients == aBroker.clients, (self.clients, aBroker.clients)
			return True
		except:
			traceback.print_exc()
			raise
